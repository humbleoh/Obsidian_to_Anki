// Subset of FiraCode-Regular.ttf (SIL Open Font License 1.1) covering only
// Unicode ranges whose East_Asian_Width is Ambiguous (U+2500-259F box-drawing
// + block elements, U+2190-21FF arrows). On CJK-locale mobile devices these
// glyphs would otherwise render at 2 cells wide, breaking ASCII-art alignment.
// Embedding the subset as base64 forces every platform to use identical
// 1-cell advance widths for these specific characters. ASCII letters and CJK
// characters fall through to the surrounding font stack (Courier New etc.).
//
// Subset generated with:
//   pyftsubset FiraCode-Regular.ttf \
//     --unicodes="U+2500-259F,U+2190-21FF" \
//     --layout-features-='*' --flavor=woff2 --no-hinting --desubroutinize
// Result: 3.5KB woff2 / 4.7KB base64.

export const TEXTGRAM_BOXART_FONT_B64: string =
    ""

// Name of the @font-face family used by textgram blocks.
export const TEXTGRAM_BOXART_FONT_NAME: string = "TextgramBoxArt"

// Unicode ranges covered by the embedded subset. Must match the unicodes
// flag used when regenerating the woff2 (see header comment).
export const TEXTGRAM_BOXART_UNICODE_RANGE: string =
    "U+2500-259F, U+2190-21FF"

// Build the <style> block that defines the @font-face. Caller is responsible
// for prepending this to the note HTML once per note (not once per block).
export function buildTextgramFontFaceStyle(): string {
    return `<style>
@font-face {
    font-family: '${TEXTGRAM_BOXART_FONT_NAME}';
    src: url(data:font/woff2;base64,${TEXTGRAM_BOXART_FONT_B64}) format('woff2');
    font-style: normal;
    font-weight: 400;
    font-display: block;
    unicode-range: ${TEXTGRAM_BOXART_UNICODE_RANGE};
}
</style>`
}
